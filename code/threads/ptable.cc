...
int PTable::ExecUpdate2(char* name, int priority) {
    // Gọi mutex->P(); để giúp tránh tình trạng nạp 2 tiến trình cùng 1 lúc.
    bmsem->P();
    ...
    // Gọi thực thi phương thức Exec của lớp PCB.
    int pid = pcb[index]->Exec2(name, index, priority);
...
//created for assignment-4 pipe function

int PTable::ExecUpdatePipe(char* name, char* pipeName, int role) {
    // Gọi mutex->P(); để giúp tránh tình trạng nạp 2 tiến trình cùng 1 lúc.
    bmsem->P();
    ...
    pcb[index]->SetPipe(pipeName, role);
